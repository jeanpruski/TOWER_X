...
const socketTransport = process.env.SOCKET_IO_TRANSPORT ?? 'auto';
if (socketTransport !== 'auto' && socketTransport !== 'polling') throw new Error('SOCKET_IO_TRANSPORT must be auto or polling.');
...
const server = await createApp({ dataFile: resolve(dataDir, 'world.json'), secret, production, databaseUrl: process.env.DATABASE_URL, origin: process.env.PUBLIC_ORIGIN ?? 'http://localhost:5173', devTools: process.env.DEV_TOOLS === '1', bots: Number(process.env.BOT_COUNT ?? 1), socketTransport });
