...
- Serveur à **30 Hz**, rendu Phaser à 60 FPS, prédiction locale et réconciliation ; interpolation des autres joueurs avec un tampon adaptatif de 100 à 250 ms.
...
Les trois premiers bonus apparaissent dans le premier tronçon ; les tronçons suivants en contiennent un chacun. Les passages ordinaires se franchissent sans bonus ; les brèches prévues pour deux demandent un partenaire humain ou bot avec le saut normal. Les effets disparaissent au retour au camp ; une reconnexion rapide retrouve l’état en cours. `BOT_COUNT=0..3` dans `.env` règle le nombre maximal de compagnons (1 par défaut) : les compagnons restent présents même quand d’autres humains arrivent, pour aider ceux qui restent en arrière. La tour vide ne simule aucun bot.
